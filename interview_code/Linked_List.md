...
[Leetcode Q237](java_src/237.删除链表中的节点.java) 237.删除链表中的节点
```
    // 本题思路：
...
[剑指Offer24](java_src/剑指Offer24.反转链表.java) 剑指Offer24.反转链表
```
easy
```

[剑指Offer52](java_src/剑指Offer52.两个链表的第一个公共节点.java) 剑指Offer52.两个链表的第一个公共节点
```
    // 本题思路：
    // A和B两个链表长度可能不同，但是A+B和B+A的长度是相同的，所以遍历A+B和遍历B+A一定是同时结束。
    // 如果A,B相交的话A和B有一段尾巴是相同的，所以两个遍历的指针一定会同时到达交点 
...
[剑指Offer36](java_src/剑指Offer36.二叉搜索树与双向链表.java) 剑指Offer36.二叉搜索树与双向链表
```
    // 本题思路：
